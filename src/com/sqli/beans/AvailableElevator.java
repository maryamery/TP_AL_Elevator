...
public class AvailableElevator extends AbstractAvailableElevator {
   ...
   @Override
   public String getIdOfFirstAvailableElevator(int floor, HashMap<String, Elevator> elevators,int numberofloors) {
      ...
      while (iterator.hasNext()) {
         ...
         int d = elevators.get(id).getState().calculDistance(floor,elevators.get(id).getCurrent_floor(),numberofloors);
         if (d < minDistance ){
            minDistance = d;
